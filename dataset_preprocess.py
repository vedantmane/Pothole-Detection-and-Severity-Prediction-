...
for path in paths:
    for file in os.listdir(os.path.join(dir, path)):
        ...
        key = cv2.waitKey(0)
        if key == ord('1'):
            cv2.imwrite(os.path.join(dir, "Pothole Data/Plain Road", file), img)
            ...
            if key_in == ord('1'):
                cv2.imwrite(os.path.join(dir, "Pothole Data/Pothole Road/Low", file), img)
                cv2.destroyWindow("Pothole: Low, Medium or High " + file)
            elif key_in == ord('2'):
                cv2.imwrite(os.path.join(dir, "Pothole Data/Pothole Road/Medium", file), img)
                cv2.destroyWindow("Pothole: Low, Medium or High " + file)
            elif key_in == ord('3'):
                cv2.imwrite(os.path.join(dir, "Pothole Data/Pothole Road/High", file), img)
                cv2.destroyWindow("Pothole: Low, Medium or High " + file)
            else:
                cv2.imwrite(os.path.join(dir, "Pothole Data/unallocated", file), img)
                cv2.destroyWindow("Pothole: Low, Medium or High " + file)
        else:
            cv2.imwrite(os.path.join(dir, "Pothole Data/unallocated", file), img)
            cv2.destroyWindow(str(count) + " " + file)
